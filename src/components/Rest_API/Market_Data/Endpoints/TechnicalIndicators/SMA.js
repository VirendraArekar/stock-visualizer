...
const SMA = (props) => {
  ...
  const processStockData = (results) => {
    ...
    const rows = results.map(result => ({
      ...
      timestamp: ConvertToTimestamp(result.t),
      volume: result.v,
  ...
  const ConvertToTimestamp = (timestamp) => {
    const dateInMilliseconds = timestamp / 1000;  // Convert from seconds
    return new Date(dateInMilliseconds).toLocaleString();
  };
  ...
  const goToAnalysis = () => {
    navigate('/smagraphpath', { state: { formData } });
  };
      ...
      <div style={styles.formContainer}>
        ...
        <div className="parameterItem">
          ...
          <input
            ...
            onChange={handleInputChange}
            placeholder="Enter window size (integer)"
            className="input"
            min="1"
          />
        </div>
